	...
	static List<String> routeAll(List<Route> routes, List<String> paths) {
		...
		for (String path : paths) {
			...
			if (staticRoutes.containsKey(path)) {
				...
				// If we couldn't find a matching static route, then check the
				// dynamic routes
				// Worst case is all routes are dynamic
				// O(m)
				for (Route dynamicRoute : dynamicRoutes) {
					// We have to check all dynamic routes unless we found one
					// that matches
					if (isFound) {
						break;
					}
					...
					// If the number of components in the request path != the
					// number of components in the dynamic route,
					// then this dynamic route isn't a match :(
					if (pathParts.length == dynamicRoutePaths.length) {
						// Step through the component in the request path and
						// check that each matches the corresponding
						// component in the dynamic route path, or that the
						// component in the dynamic route path is a
						// wildcard X
						// O(n)
